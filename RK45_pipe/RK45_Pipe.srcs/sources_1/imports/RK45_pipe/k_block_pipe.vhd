...
architecture Behavioral of k_block_pipe is
    ...
    -- Stage 2
    signal x2_raw    : STD_LOGIC_VECTOR(31 downto 0) := (others => '0'); -- C2*h+x_in  @17
    ...
    -- Stage 3
    signal x3_raw    : STD_LOGIC_VECTOR(31 downto 0) := (others => '0'); -- @17
    ...
    -- Stage 4
    signal x4_raw    : STD_LOGIC_VECTOR(31 downto 0) := (others => '0');
    ...
    -- Stage 5
    signal x5_raw    : STD_LOGIC_VECTOR(31 downto 0) := (others => '0');
    ...
    -- Stage 6
    signal x6_raw    : STD_LOGIC_VECTOR(31 downto 0) := (others => '0');
    ...
    -- h and y_in: one long pipe each, tapped at S2..S6 indices.
    -- Size = S6_ALIGN (310): covers all stage taps within bounds.
    signal h_pipe    : slv32_pipe(0 to S6_ALIGN - 1) := (others => (others => '0'));
    signal y_pipe    : slv32_pipe(0 to S6_ALIGN - 1) := (others => (others => '0'));

    -- x_n_raw alignment pipes (extra delay after the fpu_fused for each x_n)
    signal x2_pipe   : slv32_pipe(0 to S2_ALIGN - 1) := (others => (others => '0'));
    signal x3_pipe   : slv32_pipe(0 to S3_ALIGN - 1) := (others => (others => '0'));
    signal x4_pipe   : slv32_pipe(0 to S4_ALIGN - 1) := (others => (others => '0'));
    signal x5_pipe   : slv32_pipe(0 to S5_ALIGN - 1) := (others => (others => '0'));
    signal x6_pipe   : slv32_pipe(0 to S6_ALIGN - 1) := (others => (others => '0'));
...
begin
    ...
    begin
        if rising_edge(clk) then
            ...
            -- h and y_in shared tapped delay lines (length 310)
            h_pipe(0) <= h;
            y_pipe(0) <= y_in;
            ...
            -- x_n alignment pipes
            x2_pipe(0) <= x2_raw;
            for i in 1 to S2_ALIGN - 1 loop x2_pipe(i) <= x2_pipe(i-1); end loop;

            x3_pipe(0) <= x3_raw;
            for i in 1 to S3_ALIGN - 1 loop x3_pipe(i) <= x3_pipe(i-1); end loop;

            x4_pipe(0) <= x4_raw;
            for i in 1 to S4_ALIGN - 1 loop x4_pipe(i) <= x4_pipe(i-1); end loop;

            x5_pipe(0) <= x5_raw;
            for i in 1 to S5_ALIGN - 1 loop x5_pipe(i) <= x5_pipe(i-1); end loop;

            x6_pipe(0) <= x6_raw;
            for i in 1 to S6_ALIGN - 1 loop x6_pipe(i) <= x6_pipe(i-1); end loop;
    ...
    ---------------------------------------------------------------------------
    f1 : func port map (x_in => x_in, y_in => y_in, clk => clk, f => K11);
    ...
    ---------------------------------------------------------------------------
    -- x2_raw = C2*h + x_in  (fpu_fused: A*B+C = C2*h + x_in)
    ...
    -- y2 = h_delayed_42 * s2_mul1 + y_in_delayed_42
    -- h_pipe(S2_ALIGN-1) = h_pipe(41) = h delayed by 42 cycles, aligned with s2_mul1
    ...
    -- k2 = f(x2_aligned, y2)   x2_pipe(41) and y2_raw both at depth 59
    f2 : func port map (
        x_in => x2_pipe(S2_ALIGN - 1),
        y_in => y2_raw,
        clk  => clk,
        f    => K22);
